...
do_cholesky_base (Matrix & matrix)
{
...
template < typename Matrix > void
tri_solve_base (Matrix & SW, Matrix & NW)
{
...
#if 0
template < typename MatrixSE, typename MatrixSW > struct tri_schur_base_bracket_t
{
...
template < typename TriSchurBase,... > struct rec_cholesky
{
  ....TriSchurBase tsb;
  ... tsb (se, sw);
    TriSchurBase () (se, sw);
