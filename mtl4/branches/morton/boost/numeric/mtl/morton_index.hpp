...
#ifndef MORTON_INDEX_INCLUDE
#define MORTON_INDEX_INCLUDE
...
/******************************************************
 Dilated integer algorithms
 *****************************************************/

#define DilateBits(x)    ( dilate_lut[ (unsigned char)(x) ]  \
                         + (dilate_lut[ (unsigned short int)(x) >> 8 ] << 16) ) 

#define UndilateBits(x)  ( undilate_lut[ (unsigned char) (((x) >> 7)+ (x)) ] \
                         + (undilate_lut[ (unsigned char)((((x) >> 7)+ (x)) >> 16)] << 8)) 
...
/******************************************************
 Anti-Dilated integer algorithms
 *****************************************************/

#define AntiDilateBits(x)   ( anti_dilate_lut[ (unsigned char)(x) ]  \
                            + (anti_dilate_lut[ (unsigned short int)(x) >> 8 ] << 16) ) 

#define AntiUndilateBits(x) UndilateBits(x & EvenBits)


#define AntiEvenDilate(i)   (AntiDilateBits(i))
#define AntiOddDilate(i)    ((AntiDilateBits(i) <<1) + 1)
#define AntiEvenUndilate(i) (AntiUndilateBits(i))
#define AntiOddUndilate(i)  (AntiUndilateBits((i)>>1))

#define AntiEvenAdd(i,j)    ((((i) & EvenBits) + (j)) | OddBits)
#define AntiOddAdd(i,j)     ((((i) & OddBits) + (j))  | EvenBits)
#define AntiEvenSub(i,j)    (((i) - (j)) | OddBits)
#define AntiOddSub(i,j)     (((i) - (j)) | EvenBits)

/* 
   #define AntiEvenPlus1(j)    ((((j) & EvenBits) + 0xaaaaaaab) | OddBits)
   #define AntiOddPlus1(j)     ((((j) & OddBits) + 0x55555557) | EvenBits)

   a better way to do plus1 and Inc:
   use j + 1 = j - (-1).
   AntiEvenDilate(-1) = AntiOddDilate(-1) = 0xffffffff
   AntiEvenDilate(1)  = 0xaaaaaaab
   AntiOddDilate(1)   = 0x55555557 
*/ 
#define AntiEvenPlus1(j)    (((j) - 0xffffffff) | OddBits)
#define AntiOddPlus1(j)     (((j) - 0xffffffff) | EvenBits)

#define AntiEvenInc(j)      (j = AntiEvenPlus1(j))
#define AntiEvenDec(j)      (j = (((j) - 0xaaaaaaab) | OddBits))
#define AntiEvenSub1(j)     (((j) - 0xaaaaaaab) | OddBits)

#define AntiOddInc(j)       (j = AntiOddPlus1(j))
#define AntiOddDec(j)       (j = (((j) - 0x55555557) | EvenBits))
#define AntiOddSub1(j)      (((j) - 0x55555557) | EvenBits)
