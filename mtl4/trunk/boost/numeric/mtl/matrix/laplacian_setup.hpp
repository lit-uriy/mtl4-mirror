...
inline void laplacian_setup(Matrix& matrix, unsigned dim1, unsigned dim2)
{
	...
	for (unsigned j= 0; j < dim2; j++) {
	    typename Collection<Matrix>::value_type four(4.0), one(1.0);
	    unsigned row= i * dim2 + j;
	    ins(row, row) << four;
	    if (j < dim2-1) ins(row, row+1) << one;
	    if (i < dim1-1) ins(row, row+dim2) << one;
	    if (j > 0) ins(row, row-1) << one;
	    if (i > 0) ins(row, row-dim2) << one;
	}
}
