...
\req{values:}The type of the \code{N}th element of \code{T}, where
indexing is zero-based.
...
\req{Return type:} \code{const tuple\_element<N, T>::type\&}.\\
...
\hspace{\parindent} \code{t == u}, \code{t = uc}
...
\req{Return type:} \code{bool}.\\
\req{Returns:} The result of a lexicographical comparison with \code{$\odot$} between \code{t} and \code{u}, defined as:
...
\req{Note:} The above definition does not impose the requirement that 
...
or \code{ostream} is understood to mean an instantiation of 
\code{basic\_stream} or \code{basic\_ostream}, which is the type of 
...
\req{Effects:} Inserts \code{t} into \code{os} as \code{$L$t$_1$$d$t$_2$$d$...$d$t$_nR$}, where $L$ is the opening, $d$ the delimiter and $R$ the closing
character set by tuple manipulators.
Each element \code{t$_i$} is output by invoking \verb|os << t|\code{$_i$}.
A zero-elment tuple is output as $LR$, one-element tuple as \code{$L$t$_0R$}
\\
...
\req{Effects:} Extracts a tuple of the form \code{$L$t$_1$$d$t$_2$$d$...$d$t$_nR$},
...
\emph{tuple\_manip} \code{tuple\_open(char\_tupe c)}
...
the expressions \verb|out << s| and \verb|in >> s| are required to fail at compile time if
...
\req{Return type:} \code{tuple<T1, T2, ..., TN>}, where 
\begin{itemize}
\item if the non-reference type \code{Vi} of \code{ti} is an array type, 
then \code{Ti} is a reference \code{const Vi}.
\item if the non-reference cv-unqualified type \code{Vi} of \code{ti} is 
\code{any\_holder<X>}, then \code{Ti} is \code{X}.
\item otherwise \code{Ti} is the non-reference type of \code{ti}, with the
possible const qualifier removed.
\end{itemize}
...
\req{Notes:} To construct a tuple which contains a reference to a function,
 the 
