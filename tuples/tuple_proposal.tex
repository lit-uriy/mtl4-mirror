...
\emph{tuple\_manip1} \code{tuple\_open(char\_type c)}

\emph{tuple\_manip2} \code{tuple\_close(char\_type c)}

\emph{tuple\_manip3} \code{tuple\_delimiter(char\_type c)}
...
\code{reference\_wrapper<X>}, then \code{Ti} is \code{X\&}.
\item otherwise \code{Ti} is \code{Vi}.
\end{itemize}
\req{Example:} 
...
\req{Returns:} \verb|tuple<T0&, T1&, ..., TN&>(t0, t1, ..., tn)|
