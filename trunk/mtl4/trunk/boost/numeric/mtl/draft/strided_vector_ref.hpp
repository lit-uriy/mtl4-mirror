    ...
    public crtp_base_vector< strided_vector_ref<ValueRef, Parameters>, ValueRef, std::size_t >
{
  ...
  public:
    ...
    friend size_type inline num_rows(const self& v) { return traits::is_row_major<self>::value ? 1 : size(v); }
    friend size_type inline num_cols(const self& v) { return traits::is_row_major<self>::value ? size(v) : 1; }
