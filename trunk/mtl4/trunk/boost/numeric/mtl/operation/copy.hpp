...
namespace mtl {
	...
	namespace detail {
	    ...
	    template <typename MatrixDest>
	    inline void zero_with_sparse_src(MatrixDest& dest, tag::universe) {} // {set_to_zero(dest);}

	} // namespace detail

	template <typename MatrixSrc, typename MatrixDest>
	inline MatrixDest& copy(const MatrixSrc& src, MatrixDest& dest)
	{
...
#if 0
    template <typename MatrixSrc, typename MatrixDest>
    inline MatrixDest& copy(const MatrixSrc& src, tag::matrix, MatrixDest& dest, tag::matrix)
    {
    ...
    inline CollDest& copy(const CollSrc& src, CollDest& dest)
    {
