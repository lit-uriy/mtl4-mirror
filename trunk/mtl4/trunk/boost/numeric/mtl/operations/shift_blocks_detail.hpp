...
inline void copy_forward(Size& i, Size blocks, Starts const& starts, NewStarts const& new_starts, 
			 Ends const& ends, Data& data)
{
	...
	if (starts[i] > new_starts[i])
	    std::copy(&data[starts[i]], &data[ends[i]], &data[new_starts[i]]);
}

template <typename Size, typename Starts, typename NewStarts, typename Ends, typename Data>
inline void copy_backward(Size& i, Size blocks, Starts const& starts, NewStarts const& new_starts, 
			  Ends const& ends, Data& data)
{
    Size first = i;
	...
	if (ends[j] <= new_starts[j])
	    std::copy(&data[starts[j]], &data[ends[j]], &data[new_starts[j]]);
	else
	    std::copy_backward(&data[starts[j]], &data[ends[j]], &data[new_starts[j]+ends[j]-starts[j]]);
}
