...
A movable type models \ref concept_movable. There are four components of a movable type:
	...
	- Specialize the adobe::is_movable trait to declare that the type is movable.
...
{
    implementation(int x = 0) : member(x) { }
...
// model concept Regular
    ...
    movable(int x = 0) : member(new implementation(x)) { }
...
// model concept Movable
    ...
    movable(movable& x, adobe::move_ctor) : member(x.member)
    { x.member = 0; }
...
// Declare that movable models Movable
namespace adobe { template <> struct is_movable<movable> : boost::mpl::true_ {}; }
...
\brief Users can register their class as movable once they've added the appropriate constructors
and assignment operator by specializaing is_movable.
*/
...
struct move_sink : boost::enable_if<
                    ...
                    >
{
    // Movable types must provide T& operator=(T) to enable RVO
BOOST_MPL_ASSERT((boost::mpl::or_<boost::mpl::not_<is_movable<T> >, \
                                  implementation::class_has_move_assign<T> >));
};
...
/*!
\ingroup move_related
\brief move_ctor is used as a tag. For a movable class T, the constructor of the form 
  T(T&, move_ctor) will be invoked when move construction is desired.
*/
class move_ctor { };
...
*/
template <typename T>
T move(T& x, typename move_sink<T>::type = 0) { return T(x, move_ctor()); }
...
{
    ::new(static_cast<void*>(p)) T(x, move_ctor());
}
