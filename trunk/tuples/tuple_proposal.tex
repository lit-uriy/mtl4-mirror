...
\textsf{[Annex B is the list of recommended minima for implementation 
quantities.]}
...
\noindent\hspace{\parindent}  \begin{minipage}{10.5cm} \begin{verbatim}
template <class T1 = implemantation-defined, 
...
tuple<T1, T2, ..., TN> 
make_tuple(const V1&, const V2& , ..., const VN&); 
...
public:
  tuple();
  tuple(P1, P2, ..., PN); // iff N > 0

  tuple(const tuple&);

  template <class U1, class U2, ..., class UN> 
  explicit tuple(const tuple<U1, U2, ..., UN>&);
...
\noindent If \code{Ti} is a reference type then \code{Pi} is \code{Ti}, otherwise
\code{Pi} is \code{const Ti\&}.
...
template<class V1, class V2, ..., class VN>
tuple<T1, T2, ..., TN> 
make_tuple(const V1& v1, const V2& v2, ..., const VN& vn); 
\end{verbatim}
\end{minipage}

\smallskip

\noindent where \code{Ti} is \code{X\&}, if the cv-unqualified type \code{Vi} is \code{reference\_wrapper<X>}, otherwise \code{Ti} is \code{Vi}.

\smallskip 

\req{Returns:} \code{tuple<T1, T2, ..., TN>(v1, v2, ..., vn)}.\\
...
indexing is zero-based.
A diagnostic must be produced for a value of \code{I} that is out of bounds.
...
\subsubsection*{Header \code{<tupleio>} synopsis}
