...
\item A mechanism for `unpacking' tuple elements into separate variables.
\end{itemize}
...
This is a feature that cannot be added to C++ as a library.
...
element of a tuple type can be accessed using 
...
The library defines three manipulators, \code{tuple\_\-open}, 
\code{tuple\_\-close} and \code{tuple\_\-delimiter} to change, respectively, 
...
The number of elements of a non-tuple-like type is 1.
...
\item \code{tuple\_size<P>::value} is a valid integral constant expression, and must be nonnegative, and
\item \code{tuple\_element<N, P>::type} is a valid expression that designates a type whenever \code{N} is in bounds, and
\item \code{get<N>(p)}, where \code{p} is either of type \code{P} or \code{const P}, is as defined in the \emph{Element access} subclause.
\end{itemize}
...
\code{X$_i$} & the type of the $i$th element in \code{X}, where X is a tuple-like type\\
...
\req{Requires:} \code{tuple\_length<T>::value == tuple\_length<U>::value}. 
For all $i$ in \code{T},
\code{tc$_i$ == uc$_i$} is a valid expression returning a type that is
convertible to \code{bool}.\\
\req{Return type:} \code{bool}.\\
\req{Returns:} \code{true} iff {tc$_i$ == uc$_i$} for all $i$ in \code{T}.
...
\req{Notes:} The above definitions do not impose the requirement that 
...
\noindent
\begin{sf}
[ There seem to exist conditions where the converting copy constructor
and elementwise construction conflict.
...
\begin{itemize}
\item if the non-reference type \code{Vi} of \code{vi} is an array type, 
then \code{Ti} is a reference to \code{const Vi}.
\item if the non-reference cv-unqualified type \code{Vi} of \code{vi} is 
\code{any\_holder<X>}, then \code{Ti} is \code{X}.
\item otherwise \code{Ti} is the non-reference type of \code{vi}, with 
